...
@section('content')
<form class="form-group " method="POST" action="{{route("equipos.update", [$equipo->id])}}">\
    @method('PUT')
    <div class="card card-primary card-outline">
        <div class="card-header">
            <h5>
                Editar {{$equipo->marca}} {{$equipo->modelo}}
            </h5>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col">
                    <div class="form-group ">
                        ...
                        <input type="text" class="form-control  @error('marca') is-invalid @enderror" id="marca"
                            name="marca" value="{{ $equipo->marca ?? old('marca') }}" placeholder="Especifique la marca"
                            required>
                        @error('marca')
                ...
                <div class="col">
                    <div class="form-group ">
                        ...
                        <input type="text" class="form-control  @error('modelo') is-invalid @enderror" id="modelo"
                            name="modelo" value="{{ $equipo->modelo ?? old('modelo') }}"
                            placeholder="Especifique el modelo" required>
                        @error('modelo')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="col">
                    <div class="form-group ">
                        ...
                        <input type="date" class="form-control  @error('fecha_garantia') is-invalid @enderror"
                            id="fecha_garantia" name="fecha_garantia"
                            value="{{ $equipo->fecha_garantia ?? old('fecha_garantia') }}"
                ...
                <div class="col">
                    <div class="form-group ">
                        ...
                        <input type="number" class="form-control  @error('numero_serie') is-invalid @enderror"
                            ...
                            value="{{ $equipo->numero_serie ?? old('numero_serie') }}"
            ...
            <div class="row">
                <div class="col-4">
                    <div class="form-group ">
                        ...
                        <select name="cliente_id" id="cliente" class="form-control select2bs4" style="width: 100%;"
                            required>
                            <option value="" selected disabled>--Seleccione--</option>
                            @foreach ($clientes as $cliente)

                            @if ($equipo->cliente_id == $cliente->id)
                            <option value="{{$cliente->id}}" selected>{{$cliente->nombres}}, {{$cliente->apellidos}} ({{$cliente->cuil}})</option>
                            @else
                            <option value="{{$cliente->id}}">{{$cliente->nombres}}, {{$cliente->apellidos}} ({{$cliente->cuil}})</option>
                            @endif
                ...
                <div class="col-3">
                    <div class="form-group ">
                        ...
                        <select name="estado" id="estado" class="form-control" required>
                            <option value="" selected disabled>--Seleccione--</option>
                            <option value="NUEVO" @if("NUEVO"==$equipo->estado) selected @endif>NUEVO</option>
                            <option value="USADO" @if("USADO"==$equipo->estado) selected @endif>USADO</option>
                        </select>
                        @error('estado')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
            ...
            <div class="form-group ">
                ...
                <textarea name="notas_generales" id="notas_generales" cols="30" rows="10" class="form-control"
                    placeholder="Ingrese las notas generales">{{ $equipo->notas_generales ?? old('notas_generales') }}</textarea>
